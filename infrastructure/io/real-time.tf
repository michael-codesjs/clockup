resource "aws_sns_topic" "real_time_topic" {
  name         = "clock-up-real-time-${var.stage}"
  display_name = "clock-up real-time topic."
}

resource "aws_sqs_queue" "real_time_request_queue" {
  name                      = "clock-up-real-time-request-${var.stage}"
  ...
  tags = {
    ...
    Description = "clock-up ${var.stage} real-time service request queue."
  }
}

resource "aws_sqs_queue" "real_time_request_dead_letter_queue" {
  name = "clock-up-real-time-request-dead-letter-${var.stage}"
  tags = {
    Enviroment  = var.stage
    Description = "clock-up ${var.stage} real-time service request dead letter queue."
  }
}

resource "aws_sqs_queue" "real_time_response_queue" {
  name                      = "clock-up-real-time-response-${var.stage}"
  receive_wait_time_seconds = 20
  tags = {
    Environment = var.stage
    Description = "clock-up ${var.stage} real-time service response queue."
  }
  redrive_policy = jsonencode({
